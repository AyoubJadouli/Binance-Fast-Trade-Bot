    ...
    try:
        for coin in prices:
            if CUSTOM_LIST and USE_MOST_VOLUME_COINS == False:
                ...
                for coinsym in VOLATILE_VOLUME_LIST:
                    if coinsym + PAIR_WITH == coin['symbol'] and coin['symbol'].replace(PAIR_WITH, "") not in EX_PAIRS:
                        #initial_price[coin['symbol']] = { 'price': coin['price'], 'time': datetime.now()}
    ...
    try:
        for coin in volatile_coins:
            ...
            try:
                ...
                print(f"{txcolors.RED}debug {volume[coin]}")
    ...
    for coin in orders:
        ...
        if not TEST_MODE:
            ...
            coins_bought[coin] = {
                ...
                'bought_at': orderbook[symfilter(coin)]['asks'][0][0],
                'volume': volume[coin],
                'stop_loss': -STOP_LOSS,
                'take_profit': TAKE_PROFIT,
                'step_size': float(coin_step_size),
...
def renew_list(in_init=False):
    try:
        global tickers, VOLATILE_VOLUME, FLAG_PAUSE, COINS_MAX_VOLUME, COINS_MIN_VOLUME, one_time_prices
        volatile_volume_empty = False
        volatile_volume_time = False
...
async def main(loop):
    ...
    global one_time_prices
    global VOLATILE_VOLUME_LIST,orderbook
