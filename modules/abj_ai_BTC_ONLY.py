# ABJ AI MOD
print("|||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||")
print("||||||||||||||||||||||     AI Crypto Bot by ABJ vBTC ONLY  |||||||||||||||||||||||")
print("|||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||")
...
Normalization_File= "AI/BTC-USDT-tp77_w100_max20min_Norm_v1.json"
Model_FileName= "AI/BTC-USDT-tp77_w100_max20min_Model_v1.hdf5"
...
def Buy_Dessision_DataFrame(input):
    print("making prediction step0")
    #A=np.array(input)
    #A = A.reshape(1,A.shape[0])
    print("making prediction step1")
...
def instant_full_data(pair,exchange=ex,window=WINDOW_SIZE):
    ...
    pdata=instant_pair_data("BTC/USDT",exchange=exchange,window=WINDOW_SIZE)
...
async def async_instant_full_data(pair,exchange=ex,window=WINDOW_SIZE):
    ...
    #Timestamp=pd.to_datetime(ex.fetchTime(),unit='ms')
    await asyncio.wait([task_pdata, task_whattime])
...
#################################### New ##############################3
    try:
        ...
        DF['Note']= np.float32(Buy_PLUS(DF))+np.float32(PRESSISION)
