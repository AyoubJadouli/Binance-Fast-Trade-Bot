...
global parsed_config, creds_file, access_key, secret_key, parsed_creds
...
PROXY_HTTP = parsed_config['script_options'].get('PROXY_HTTP')
PROXY_HTTPS = parsed_config['script_options'].get('PROXY_HTTPS')
...
SELL_PORCENTAGE_IF_NOT_TIME_MAX = 0.3
SELL_PORCENTAGE_IF_TIME_MAX = 0.02
    ...
    try:
        ...
        for coin in prices:            
            if coin['symbol'] == symbol:
                initial_price = float(coin['price'])
                break
...
def load_json(p):
    try:
        ...
        write_logprint("Error on line {}".format(sys.exc_info()[-1].tb_lineno))
...
def analyze(pairs):
    ...
    for pair in pairs:
        ...
        try:
            ...
            #CLOSE_old = round(coins['close'].iloc[-1],5)
            CLOSE = get_price(pair)
            #print(f'CLOSE= {CLOSE} CLOSE_old= {CLOSE_old}')
            #print(f'EMA200_1MIN= {EMA200_1MIN}')
            #print(f'{SIGNAL_NAME}: {txcolors.BUY}{pair} - RSI10 = {format(RSI10,"6f")}{txcolors.DEFAULT}')
            print(f'{SIGNAL_NAME}: {txcolors.BUY}{pair} - RSI2 = {format(RSI2,"6f")}{txcolors.DEFAULT}')
            ...
            if SELL_ON_SIGNAL_ONLY == True:
                ...
                if last_price != 0:
                    SELL_TYPE1 = ((last_price*SELL_PORCENTAGE_IF_NOT_TIME_MAX)/100)+last_price
                    SELL_TYPE2 = ((last_price*SELL_PORCENTAGE_IF_TIME_MAX)/100)+last_price
                    
                    #print(f'{pair} - last_price={last_price} timeHold={timeHold}')
                    time_held = timedelta(seconds=datetime.now().timestamp()-int(str(timeHold)[:10]))
                    #print(f'timeHold= {timeHold} time_held= {time_held}')
                    print(f'{SIGNAL_NAME}: {txcolors.DIM}{pair} last_price= {last_price} / SELL_TYPE1= {SELL_TYPE1} - SELL_TYPE2= {SELL_TYPE2} time_held={round(time_held.total_seconds(),0)}{txcolors.DEFAULT}')
                    #print(f'{pair} - time_held= {round(time_held.total_seconds(),0)}')
                    if round(time_held.total_seconds(),0) >= TIME_MAX and TIME_MAX != 0:
                        #timemax = True
                        sellSignal = CLOSE >= SELL_TYPE2
                    else:
                        #timemax = False
                        #sellSignal = crossunder(RSI2,RSI_MAX) and last_price !=0 and last_price < CLOSE
                        sellSignal = last_price !=0 and CLOSE >= SELL_TYPE1
                        ...
                        #print(bought)
                    if sellSignal == True:
                        write_log(f'{SIGNAL_NAME}: {txcolors.BUY}{pair} - Sell Signal Detected RSI10={round(RSI2,5)} last_price={last_price} CLOSE={round(CLOSE,5)} timemax= {round(time_held.total_seconds(),0)}/{TIME_MAX} sellSignal= {sellSignal}{txcolors.DEFAULT}')
                        #if SELL_ON_SIGNAL_ONLY == True:
